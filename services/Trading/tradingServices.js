...
export const createTrade = async (
  ...
  try {
    ...
    const requiredMargin = parseFloat(tradeData.requiredMargin);
...
export const updateTradeStatus = async (
  ...
  try {
          ...
          if (!mt5CloseResult.success) {
            if (mt5CloseResult.error.includes("Position not found")) {
              ...
              sanitizedData.profit =
                order.type === "BUY"
                  ? (sanitizedData.closingPrice - order.openingPrice) *
                    order.volume
                  : (order.openingPrice - sanitizedData.closingPrice) *
                    order.volume;
            } else {
              throw new Error(
    ...
    if (sanitizedData.orderStatus === "CLOSED") {
      sanitizedData.closingPrice = clientClosingPrice;
    }
    ...
    if (lpPosition) {
      ...
      order.lpPositionId = lpPosition._id; // Update order with lpPositionId
      await order.save({ session: mongoSession });
    } else {
