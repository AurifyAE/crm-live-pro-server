...
export const createTrade = async (adminId, userId, tradeData, session = null) => {
  ...
  try {
    ...
    const priceData = await mt5Service.getPrice(mt5Symbol);
    console.log(`Market Price for ${mt5Symbol}: ${JSON.stringify(priceData)}`);
    if (!priceData || !priceData.bid || !priceData.ask) {
      throw new Error(`No valid price quote available for ${mt5Symbol}`);
    }
